...
async function updateMemberRoles(guild) {
  ...
  for (const [, member] of members) {
    ...
    const daysInServer = Math.floor(
      (now - member.joinedTimestamp) / (1000 * 60 * 60 * 24),
    );
    ...
    if (!member.roles.cache.has(targetRole.roleId)) {
      ...
      console.log(
        `🦆 ${member.user.username} → cargo atualizado (${daysInServer} dias)`,
      );
      atualizados++;
    }
  }
  ...
  console.log(
    `✅ Verificação concluída — ${atualizados} membro(s) atualizado(s)`,
  );
}
